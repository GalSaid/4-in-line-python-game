...
class AIBoard(Board):
    ...
    def score_position(self, piece, col):
        ...
        for row in range(self.rows - 3):
            for c in range(self.cols - 3):
                inside_the_window = False
                window = []
                for i in range(AIBoard.WINDOW_LENGTH):
                    if r == row + i and col == c + i:  # Checks whether the position that is given a score will be in
                        # the window
                        inside_the_window = True
                    window.append(self.board[row + i][c + i])
                if not inside_the_window:
                    continue
                score += AIBoard.evaluate_window(window, piece)
        ...
        for row in range(self.rows - 3):
            for c in range(self.cols - 3):
                ...
                window = []
                for i in range(AIBoard.WINDOW_LENGTH):
                    if r == row + 3 - i and col == c + i:  # Checks whether the position that is given a score will
                        # be in the window
                        inside_the_window = True
                    window.append(self.board[row + 3 - i][c + i])
                if not inside_the_window:
                    continue
                score += AIBoard.evaluate_window(window, piece)
        return score
